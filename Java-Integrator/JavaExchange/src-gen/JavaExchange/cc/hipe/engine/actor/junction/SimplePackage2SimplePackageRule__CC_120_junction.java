package JavaExchange.cc.hipe.engine.actor.junction;
...
public class SimplePackage2SimplePackageRule__CC_120_junction extends GenericJunctionActor{
	private Map<Object, Collection<HMatch>> simplePackage_4_AttrMap = HiPEMultiUtil.createMap();
	...
	protected void registerMatchForAttributeChanges(HMatch match) {
		...
		Collection<HMatch> simplePackage_0_Matches = simplePackage_4_AttrMap.get(matchObjects[0]);
		if(simplePackage_0_Matches == null) {
			simplePackage_0_Matches = new LinkedList<>();
			simplePackage_4_AttrMap.put(matchObjects[0], simplePackage_0_Matches);
		}
		
		simplePackage_0_Matches.add(match);
		
		Collection<HMatch> simplePackage_1_Matches = simplePackage_4_AttrMap.get(matchObjects[1]);
		if(simplePackage_1_Matches == null) {
			simplePackage_1_Matches = new LinkedList<>();
			simplePackage_4_AttrMap.put(matchObjects[1], simplePackage_1_Matches);
		}
	...
	protected void deregisterMatchForAttributeChanges(Set<HMatch> matches, HMatch match) {
		Object[] matchObjects = match.getNodes();
		Collection<HMatch> matches_0 = simplePackage_4_AttrMap.get(matchObjects[0]);
		if(matches_0 != null) {
			matches.remove(match);
		}
		Collection<HMatch> matches_1 = simplePackage_4_AttrMap.get(matchObjects[1]);
		if(matches_1 != null) {
			matches.remove(match);
		}
	}
	...
	protected void changeAttribute(AttributeChanged<HMatch> message) {
		...
		if(obj instanceof org.xtext.simplejava.simpleJava.SimplePackage) {
			if(simplePackage_4_AttrMap.containsKey(obj)) {
				for(HMatch attr_match : simplePackage_4_AttrMap.get(obj)) {
